...
const Random = {
	...
	NumberBetween: function(pMin: number, pMax: number): number {
		// * Checks
		if ((pMin >= pMax) || (pMin < 0)) throw new RangeError(`NumberBetween: fix pMin or pMax, currently: ${pMin} and ${pMax}`);
